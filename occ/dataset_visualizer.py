...
from OCC.Core.Quantity import Quantity_NOC_WHITE, Quantity_Color, Quantity_TOC_RGB
...
def display():
    ...
    occ_display.View.SetBackgroundColor(Quantity_Color(1.0, 1.0, 1.0, Quantity_TOC_RGB))
...
colors_path = Path(__file__).resolve().with_name('colors.json')
with open(colors_path, 'r', encoding='utf-8') as file:
    names = json.load(file)
    colors = {}
    for name in names:
        try:
            colors[name] = Quantity_Color(names[name])
        except RuntimeError:
            d = hashlib.md5(name.encode('utf-8')).digest()
            r = (d[0] / 255.0) * 0.8 + 0.2
            g = (d[1] / 255.0) * 0.8 + 0.2
            b = (d[2] / 255.0) * 0.8 + 0.2
            colors[name] = Quantity_Color(r, g, b, Quantity_TOC_RGB)
...
shape_paths = glob.glob(osp.join(args.dataset_dir, '*.step'))
